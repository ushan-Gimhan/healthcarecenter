...
public class SessionBOImpl implements SessionBO {
    ...
    public boolean save(SessioonDTO patient) {
        return sessionDAO.save(new TherapySession(patient.getSId(),patient.getSessionCount(),patient.getTime(),patient.getDate(), patient.getStatus()));
    }

    @Override
    public boolean update(SessioonDTO patient) {
        return sessionDAO.update(new TherapySession(patient.getSId(),patient.getSessionCount(),patient.getTime(),patient.getDate(), patient.getStatus()));
    }
